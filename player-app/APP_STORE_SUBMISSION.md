...
Candidate state: [PR #27](https://github.com/sahusgupta/orbit/pull/27) squash-merged at `8d086bfd689b7f0af1037468c0764d16b32d77bc` on `2026-09-07T00:37:51Z`, after explicit user approval and all PR checks passed. Reviewed head `113823ca1f7bad430bbb5943559533139126b654` has the identical source tree. This follows [PR #26](https://github.com/sahusgupta/orbit/pull/26), merged at `9458a6a50567c879b537631b9b25fb76e89b4133`. All 13 local checks, general PR CI, GitGuardian, and native QA passed. Native QA includes both iPhone sizes and accessibility-large, with 15 verified screenshots; its first attempt's pre-install boot failure remains documented. [Actual-merge CI 34070422400](https://github.com/sahusgupta/orbit/actions/runs/34070422400) and the fresh standalone Web build passed. The first API attempt was deleted after restoring an unexpected secondary-alias assignment. The separately approved final merged candidate `dpl_3ChFg6JQ9e3DjfGpwT2vMe9iskcZ` is retained and build-verified, with all live aliases preserved in the final checks. Runtime verification, signed device build, TestFlight upload, and App Review submission remain incomplete. See [release execution](../docs/agent/tasks/player-release-execution.md), the [release manifest](./submission/2026-09-06/release-manifest.json), and the [scoped production proposal](../docs/agent/tasks/player-production-mutation-proposal.md).

The separately approved credential diagnostic completed its intended install-time stop and was deleted without alias changes. Current managed credential identity is resolved in the release manifest. The later retained API candidate is a separate completed operation; no signed iOS artifact exists.
...
| User Content — Other User Content | Saved game/stakes/favorite/availability preferences and user-entered availability text | When a signed-in user saves or sends the applicable fields | Yes | No | App functionality and product personalization |
| User Content — Customer Support | Content the user sends after opening the hosted support route | `EXTERNAL GATE — confirm actual support-provider intake and retention`; native has no support-content form/API | Determine from actual intake | No | Support/App functionality if collected |
| Purchases — Purchase History | Selected membership option ID/name/price/duration, request/status, and later venue-recorded payment/history | Yes when membership flows are used; Apple includes purchase tendencies | Yes | No | Membership functionality; no checkout or payment credential is collected by iOS v1 |
| Usage Data — Product Interaction | Membership, waitlist, seat/check-in, tournament-interest, favorite/preference activity, and timestamps | Yes when authenticated actions are used | Yes | No | App functionality, integrity, security, and product personalization |
...
The app-owned privacy manifest declares Name, Email Address, Phone Number, Physical Address, Coarse Location, Other User Content, User ID, Purchase History, Product Interaction, and Other Data Types with the linking, tracking, and purpose mapping above. Local-only profile data that never leaves the device is not marked as collected. The app does not save or upload the ID image, raw PDF417 barcode, or document number. It does not sell data, use data for behavioral advertising, or track people across other companies’ apps or websites.

`PaymentInfo` remains an explicit external classification gate. Orbit receives no card, bank, payment credential, or user-selected instrument; the native request sends only the server-forced constant `in-person`, which the repository classifies as a request channel while Purchase History covers the selected plan/price/status. Before submission, the privacy owner must record that interpretation against Apple’s broad “form of payment” example or add linked, non-tracking Payment Info for App Functionality. Do not describe Orbit as receiving card or bank data either way.
...
App Check privacy addendum: the native candidate uses Apple App Attest through Firebase, while Player Web uses reCAPTCHA Enterprise when configured. Apple/Google process app/device attestation material and short-lived tokens for security; protected services receive the token with requests. The API does not store tokens in application records or logs. SDKs cache tokens on device, and Player Web uses a temporary route-guard cookie removed on sign-out. No Analytics, Crashlytics, or advertising SDK was added. Include this processing and the resolved native SDK manifests in the final App Privacy approval; see [App Check architecture](../docs/architecture/PLAYER_APP_CHECK.md) and the [manifest audit](./PRIVACY_MANIFEST_AUDIT.md).
...
- [x] The same disposable prebuild and scan pass on exact pushed SHA `113823ca1f7bad430bbb5943559533139126b654` in Ubuntu pull-request CI.
- [x] Pull-request CI is green on exact reviewed PR #27 head `113823ca1f7bad430bbb5943559533139126b654`, including all three native flows.
...
| Shared production rate limit | Release engineer | Deploy the transactional Firestore limiter and `orbitRateLimits.expiresAt` TTL policy, then verify concurrent replicas and failure handling on the candidate | Repository concurrency/restart/expiry/failure tests plus exact deployment, TTL status, and smoke evidence; see `../apps/api/README.md` |
...
| Privacy data classification | Privacy/legal owner | Confirm the app-owned ten-type baseline, decide whether the constant `in-person` request channel requires `PaymentInfo`, and classify actual support/provider/IP/device/diagnostic retention | Dated decision record plus a signed-archive-to-App-Store-Connect reconciliation with no unresolved category |
| Age rating and privacy answers | Account Holder/privacy owner | Submit answers matching the final signed binary and approved classification | App Store Connect answer export/screenshots and reviewer sign-off |
