...
describe('profile import domain boundary', () => {
  ...
  it('imports the club CSV layout including member numbers, contact preferences, and split time totals', () => {
    ...
    expect(profileFromImportedRecord({
      ...
      firstName: 'Safe',
      lastName: 'Player'
    }, context)).not.toHaveProperty('hasSSN');
    const placeholderProfile = profileFromImportedRecord({
    ...
    expect(placeholderProfile).toMatchObject({ phone: '' });
    expect(placeholderProfile).not.toHaveProperty('hasSSN');
    expect(profileFromImportedRecord({
      firstName: 'Formatted',
      lastName: 'Phone',
