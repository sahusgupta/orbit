...
import { getAccountKeyFromAccess, isFutureDate, safeAccountKeyPart } from '../../../domain/licensing';
import {
...
export const useManagementPilotAccessRefresh = ({
  ...
  useEffect(() => {
    ...
    const refresh = async () => {
      ...
      const serverExpiresAt = serverLicense?.expiresAt;
      const serverAccountKey = safeAccountKeyPart(serverLicense?.accountKey || serverLicense?.licenseId || '');
      const currentAccountKey = getAccountKeyFromAccess(access);
      const signedLicenseId = safeAccountKeyPart(access.licenseId || '');
      ...
      if (
        ...
        !result.active ||
        !serverExpiresAt ||
        !isFutureDate(serverExpiresAt) ||
        serverLicense.status !== 'active' ||
        !serverAccountKey ||
        serverAccountKey !== currentAccountKey ||
      ...
      setState((current) => {
        ...
        if (currentAccess.expiresAt === serverExpiresAt && currentAccess.serverManaged) return current;
        const next = {
          ...current,
          settings: {
            ...current.settings,
            pilotAccess: {
              ...
              issuedTo: serverLicense.issuedTo || currentAccess.issuedTo,
              licenseId: serverLicense.licenseId || currentAccess.licenseId,
              serverManaged: true
            }
          }
        };
