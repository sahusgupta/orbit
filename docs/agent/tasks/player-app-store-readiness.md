...
| Full local verification | Complete with documented platform boundary | Clean installs, focused suites, `npm run verify`, browser suites, release/security gates, Expo config/check/Doctor/export, artwork, bundle scans, and Firestore Emulator tests pass. Disposable iOS prebuild/native inspection is correctly delegated to Ubuntu CI because the local host is Windows. |
| Pull request / CI | Pending | Review, focused commits, push, PR, and green checks on the exact SHA. |
...
| Release configuration | `npx vitest run scripts/player-release-config.test.js` | Pass: 1 file / 4 tests. Production origin/flag checks fail closed, redact supplied values, and remove unneeded map/audio configuration. |
...
| Final local matrix | `npm run verify` with production access disabled and an unreachable local API | Pass: four TypeScript boundaries; 211 test files across root/API/Player/Player Web/sales map with 1,511 passing and 7 skipped tests; Player Web, sales-map, and desktop production builds. Existing ExcelJS `eval` and large-chunk warnings remain. |
| Rendered browser checks | `npm run e2e:management`; `npm run e2e:public`; `npm run web:e2e` | Pass: isolated management flow; eight public pages on desktop/mobile; 15 Player Web routes across six viewports (90 screenshots) plus 42 interaction checks, with clean route metadata and no reported browser failures. The optional landing-parity harness was not asserted because its external reference app was not supplied. |
...
| Expo/native local gates | `npm run player:config:verify`; `npm run player:expo:check`; `npm run player:expo:doctor`; `npm run player:assets:verify`; `npm run player:export:ios` | Pass: production config/permissions, dependency alignment, Expo Doctor 18/18, artwork, Hermes and inspectable exports, and source/bundle exclusions. Windows cannot generate the disposable iOS project; CI owns that native inspection. |
...
- Managed iOS prebuild inspection must run on Linux/macOS CI. This Windows checkout can verify config and export, but its result must not be described as a signed build.
