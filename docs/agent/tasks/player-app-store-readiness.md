...
| Pull request / CI | Repository CI complete; external security disposition pending | PR #25 is open. The full GitHub Actions matrix passes on the exact pushed code SHA; the final documentation-only SHA must repeat that result. GitGuardian still requires its workspace owner to resolve historical synthetic test-fixture detections in the dashboard without weakening scanning or rewriting history. |
