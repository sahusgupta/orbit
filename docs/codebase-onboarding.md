...
Settings **Data** is the only bulk-player import surface. `src/domain/profileImport.ts` owns pure CSV/XLSX-row and pasted JSON/text profile normalization, canonical profile construction, duplicate filtering, and companion-link enrichment. `src/lib/profileWorkbookImport.ts` applies bounded XLSX archive checks and normalizes exporter-specific SpreadsheetML namespace prefixes before ExcelJS decoding. `src/main.tsx` retains browser file selection, user feedback, and persistence orchestration.

Government-ID capture remains a single-player workflow under Players. `src/domain/governmentId.ts` extracts only name, birth date, address, and calculated age from scanner or OCR text. `src/features/profiles/governmentIdImage.ts` tries an on-device PDF417 read first and then the bundled offline OCR engine for JPG, PNG, or WebP images. Raw scans, barcode payloads, OCR text, and image bytes are not added to application state; every image-derived identity remains pending until normal staff review.
