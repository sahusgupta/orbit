...
| --- | ---: | --- |
| `player-app/src/PlayerApp.tsx` | 953 | Expo composition shell: navigation/filter state, domain selectors, feature assembly, shared sheets/modals, two route-local effects, and 15 referenced header/content/tab styles. Storage, native services, subscriptions, identity, purchases, and feature workflows have focused owners. |
| `src/main.tsx` | 3,514 | Management composition root with five application-wide state declarations, cross-feature effects, command invocation, persistence calls, dialogs, and typed route assembly. Feature-local drafts and presentation models have focused owners. |
...
REF-025 split the 1,328-line Player data adapter into hosted/local HTTP adapters, focused Firebase Auth/account/profile/tournament/private-game/request/club repositories, one live-club subscription owner, runtime/legacy decoders, and snapshot transforms. `orbitSyncApi.ts` is now a stable 66-line re-export facade; the largest new data owner is 192 lines. Raw HTTP values are decoded from `unknown`, while REF-024's profile, tournament, and private-game malformed-record policies remain explicit compatibility functions. The final focused gate passes 4 files / 48 tests, the Player data/domain gate passes 9 files / 73 tests, and full verification passes 79 files / 461 tests plus the unchanged 1,956-module build.

REF-026 replaced renderer Firebase cloud-state, membership-request, waitlist-request, and Auth-error assertions with `firebaseClubDecoders.ts`. The decoder validates persisted collection containers, constructs canonical requests from `unknown`, preserves discriminator-less legacy records and join/interested fallbacks, and exposes only typed marker reads before duplicate/applied short-circuits. Firestore paths, protocol-v2 publication, identity rules, acknowledgements, and retry policy are unchanged. Full verification passes 79 files / 465 tests; the decoder adds one transformed module and moves the main renderer chunk to 1,009.54 kB (301.46 kB gzip).

REF-027 moved the API runtime's duplicated Firebase Admin credential/app bootstrap into `services/firebaseAdmin.js` and its duplicated Stripe construction into `services/stripeClient.js`. Identity, license, and payment services now share those lazy singleton owners; JSON/base64 precedence, the license-only explicit credential-file read, Firebase default discovery, the Stripe missing-key error, routes, webhooks, authorization, and per-club scope remain unchanged. Fake-only characterization and direct provider tests cover the dependency seams without reading credentials or calling providers. Full verification passes 80 files / 470 tests and the unchanged 1,957-module renderer build.
