...
2. Dispatch with `promote=false` and a blank `promotion_confirmation` to build the candidate.
...
5. As the sole maintainer, review the run, attestations, checksums, smoke output, change scope, and operational readiness before starting a second dispatch. A canary is promoted as a GitHub prerelease; normal clients have prerelease updates disabled. Validate it only on explicitly selected non-production/canary workstations.
6. Dispatch the same reviewed inputs with `promote=true` and `promotion_confirmation=PROMOTE`. The build is reproduced and its checksums are reverified, then the `production-release` environment records GitHub release creation. Promotion failure never falls back to an unverified file.
...
4. Run every normal gate. Start with canary, verify authoritative revision continuity, publication outbox recovery, authentication/session behavior, and critical management/public paths, then complete the same explicit solo-maintainer promotion checklist.
...
`apps/api/vercel.json` disables Vercel Git auto-deployments. Do not re-enable branch previews or automatic production aliasing while Preview carries an unverified Firebase Admin credential. From a clean checkout of the exact merged SHA, explicitly link `apps/api` to the canonical `orbit_app` project, create a production-target candidate with aliases withheld, inspect its build/runtime, run read-only health and public-route smokes, verify the owned alert route, and only then promote that exact deployment. Player Web and the public/download site follow the same candidate-withheld-alias pattern against their canonical projects.
...
Do not promote if verification is red, checksums/attestation are absent, the candidate source or version differs, database compatibility is unproven, the owned alert path is unverified, critical smokes are noisy, the candidate evidence has not been reviewed, or the typed solo-maintainer confirmation is absent. Record the failure through the incident runbook; never repair a release by replacing bytes under an existing version.
