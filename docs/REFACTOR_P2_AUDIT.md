...
**Repository implementation state:** COMPLETED, except the human-only credential inventory/rotation/removal action in SEC-017 remains open and prevents a final P2 completion claim.

| Finding / requirement | Implementation evidence | Verification |
| --- | --- | --- |
| SEC-005; 6.1, 6.29 | `electron/staffAuthorization.cjs` verifies PBKDF2 and characterized legacy staff PIN hashes against authoritative cached state, issues bounded role-scoped sessions, locks after repeated failures, and records redacted authorization telemetry. Privileged Electron handlers and renderer commands require the trusted authorization result instead of accepting selected renderer role state. | Staff verification, lockout, expiry, role-scope, IPC, and management command tests. |
| SEC-006, SEC-016; 6.26, 6.28, 6.31 | The dashboard now exchanges an owner-only credential for a short-lived HMAC session in an HttpOnly, Secure-in-production, SameSite=Lax cookie. Mutations require a CSRF header; SSE and shared auth no longer accept query credentials. Machine credentials are configured as tenant/capability/expiry-scoped, rotatable records, and the legacy shared key path is explicitly disabled by default. | API authentication/security tests and dashboard source contract tests; production cookie and legacy-key fail-closed cases. |
| SEC-007, SEC-008; 6.3, 6.6, 6.21, 6.25 | `apps/api/src/http/security.js` applies explicit same-origin/configured-origin CORS, security headers, trusted-proxy-aware endpoint quotas, multipart rejection, and connection limits. Loopback bypass now requires an explicit non-production, non-hosted development flag. Public health is minimal and privileged diagnostics require owner authorization. | Adversarial origin, proxy, rate-limit, header, upload, health, and loopback cases in API security/route tests. |
| SEC-009; 6.28 | Electron cache persistence uses OS `safeStorage` encryption and removes characterized cleartext state/projection copies after migration. Player restricted profile/auth material uses Expo SecureStore with device-only availability and a volatile fail-closed fallback. Production browser management state is memory-only; Firebase Auth explicitly uses memory persistence. Sessions are idle- and absolute-bounded. | Electron encrypted-store/migration tests, Player storage contract/core tests, browser production-injection tests, and licensing session tests. |
| SEC-010; 6.7, 6.8, 6.20, 6.23, 6.30 | Player email identity requires a 12-character password and verified email, exposes generic provider-managed reset/resend flows, and fails protected API access closed until verified. Phone identity uses Twilio Verify through rate-limited API endpoints and receives a Firebase custom token only after a short-lived phone-bound server challenge verifies the OTP. Recent authentication is required for destructive account action. | Player identity/storage boundary tests and API phone-auth verification, replay, expiry, mismatch, and rate-limit tests. |
| SEC-011 | `apps/api/src/accountDeletionService.js` implements a durable, resumable deletion job whose retention policy must be explicitly configured. It deletes or pseudonymizes state, requests, telemetry, payment identity, Firebase projections/operational records, and finally Firebase Auth; the Player UI reports retained categories. It fails closed rather than inventing legal retention meaning. | Account-deletion policy, ordering, retry, pseudonymization, failure-resume, and retained-category tests. |
| SEC-013, SEC-014; 6.2, 6.5, 6.9, 6.16, 6.32 | A shared data-protection boundary hashes identifiers, recursively redacts sensitive fields, bounds diagnostic detail, removes current-user/raw production stacks, and maps public failures to stable codes. API health and errors no longer disclose filesystem/provider detail; public and Player DTOs are explicit allowlists. The data-classification policy is recorded in `docs/architecture/DATA_CLASSIFICATION.md`. | Redaction, stable-error, health-detail authorization, payload-pruning, and source contract tests. |
| SEC-015; 6.1, 6.27 | Pilot venue identity is derived from the authenticated tenant; conflicting body venue IDs are rejected and device IDs are tenant-namespaced before telemetry persistence. | Cross-tenant heartbeat/update/telemetry/error adversarial route tests. |
| SEC-017 | `scripts/check-sensitive-paths.cjs` and CI fail when credential-shaped or debug artifacts exist inside the project tree, including ignored files, while reporting path names only. The current check detects 11 existing operator-owned paths. Per repository safety policy, their contents were not opened and they were not copied, moved, removed, or rotated. A human with credential authority must inventory validity, rotate any live material, and remove it from the synced project directory. | `npm run security:paths` correctly exits nonzero and reports only paths. This is an open external remediation, not evidence that the artifacts are safe or that rotation is complete. |
| SEC-018; 6.4, 6.29 | Electron validates sender frame/origin and channel payloads, authorizes privileged actions in the main process, narrows SMS count/body/recipient shape with rate/concurrency limits, and allowlists external HTTPS hosts. The preload exposes bounded methods only. | Electron compiler-boundary, IPC sender/payload, staff authorization, SMS cap, and external navigation tests. |
| SEC-019; 6.11, 6.17-6.19, 6.23 | Local backup/key/CSV/XLSX flows enforce pre-read byte limits, allowed extensions, MIME/signature/structure checks, and reject malformed or renamed content. The API rejects multipart input because no remote upload feature exists; the classification policy requires private non-executable quarantine before one may be introduced. | File import signature/size/structure tests and API multipart rejection tests. Current absence of upload remains distinct from the future storage safeguard. |

Stage 3 dependency disposition:

- Added exact `expo-secure-store@15.0.8` to the Player native app for OS-protected, device-only storage of the minimal restricted profile. The browser/runtime fallback is deliberately volatile rather than cleartext persistent storage.
- No SMS, Firebase, email, payment, hosted API, or other production provider was contacted. Twilio Verify and Firebase custom-token flows were verified with injected test adapters only.

Stage verification on 2026-08-11:

- Root aggregate TypeScript: **passed**.
- Player strict TypeScript: **passed**.
- Full Vitest discovery: **510/510 passed across 94 files**. The OneDrive reparse-point discovery defect recorded as REL-012 was corrected without weakening discovery coverage.
- Desktop renderer production build: **passed**, 1,958 modules. Main entry output is 746.79 kB (232.47 kB gzip), with the 412.05 kB shared entry and 1,066.53 kB ExcelJS chunk still present for Stage 4 optimization.
- Sensitive-path prevention check: **correctly failed on 11 existing operator-owned paths**. Repository prevention is implemented; credential validity, rotation, and removal remain an authorized-human security action and were not falsely recorded as complete.
- No production service, datastore, Firebase project, identity/payment/SMS provider, deployment, domain, certificate, or secret was accessed or changed. The tracked `data/orbit-api.sqlite3` artifact was not opened or modified.

### Stage 4 - Reliability, Performance, Network, and Database Efficiency

**Repository implementation state:** COMPLETED for the authorized Stage 4 scope. Verification of an externally owned operational-alert destination and representative production timings remains external follow-up; release/deployment wiring remains Stage 8 scope. SEC-017 remains open as the human-only credential action recorded above.

| Finding / requirement | Implementation evidence | Verification |
| --- | --- | --- |
| REL-004; 6.22 | Desktop root and lazy-route recovery boundaries preserve navigation, show a redacted incident ID, and provide retry/restart actions. Player has a native root boundary. Predictable lazy and account-hydration waits render labelled skeleton states instead of a blank view or premature onboarding. | `src/components/RecoveryBoundary.test.tsx`; updated root and presentation characterization coverage. |
| REL-006, REL-007, REL-011, PERF-011; 5.7, 5.8, 5.11, 5.15, 5.16 | Player mutations use stable mutation IDs and in-flight locks. Reversible waitlist actions update optimistically and roll back on failure; payment, identity, membership, and other sensitive effects remain server-confirmed. Player reads use a 12-second per-attempt timeout, caller cancellation, GET-only bounded retry/deduplication, stale-result guards, and actionable retry/error UI. | `player-app/src/data/api/boundedFetch.test.ts`, Player boundary/lifecycle tests, and presentation characterization tests. |
| REL-008, REL-012 | The management smoke harness now launches an isolated localhost Vite target with Firebase sync disabled and an unreachable local API. It covers profile creation, table start, and seating, and CI installs Playwright Chromium and runs the same smoke after the normal verification gate. Source discovery treats OneDrive reparse-point files consistently without dropping required tests. | `npm run e2e:management` passed; full discovery passed 99 files/528 tests; `.github/workflows/ci.yml`; `scripts/run-management-smoke.mjs`. |
| REL-009 | SSE events carry IDs, replay from a bounded 250-event ring using `Last-Event-ID`, explicitly signal replay reset, heartbeat idle connections, cap concurrent clients, handle backpressure/cleanup, and require periodic session reauthorization. The dashboard refreshes durable data after connection readiness or replay reset. | `apps/api/src/http/liveUpdates.test.js` covers replay, reset, capacity, and backpressure cleanup. |
| REL-010; 6.12, 6.24 | API errors, auth-abuse thresholds, and publication retry/drain failures emit bounded, redacted structured alerts with request correlation. An optional HTTPS webhook is timeout-bounded and host-allowlisted; no destination is fabricated when it is not configured. The runbook records response roles and the still-unverified external destination/owner. | `apps/api/src/http/operationalAlerts.test.js`; `docs/operations/INCIDENT_RESPONSE.md`. External destination ownership remains unverified and is not recorded as operational completion. |
| PERF-002, PERF-003; 5.10, 5.12, 5.19 | Player discovery now uses one authenticated, paginated API request whose state page is produced by two database queries for at most 50 venues. One shared club commit-marker listener replaces per-club listener fan-out; fallback polling is deduplicated and bounded. Firestore fallback reads use explicit limits and private games use an indexed status/order query. | Player boundary tests assert one shared listener and one request. Database pagination tests cover stable paging; `player-app/firestore.indexes.json` records the matching private-game index. The former normal path grew as `1 + 4N` reads plus `N` API calls for clubs and `1 + 2N` reads for tournaments. |
| PERF-005; 5.13 | Management Firebase Auth/Firestore is dynamically loaded behind `src/lib/firebaseAuthFacade.ts`; route views, QR scanning, and ExcelJS stay deferred. `scripts/measure-renderer-bundle.mjs --check` enforces a 620,000-byte/190,000-gzip initial-JavaScript budget and rejects source maps. | Audited initial entry: 914,989 bytes/281,186 gzip. Stage 4 entry: 558,814 bytes/172,314 gzip. Deferred chunks: Firebase 363,322/111,512 gzip, QR 412,054/107,682 gzip, ExcelJS 1,066,528/268,727 gzip. Exact measurements are reported without an invented improvement percentage. |
| PERF-007; 5.14, 5.18 | SQLite/PostgreSQL schemas add query-matched composite indexes. Clients, update events, telemetry, errors, venues, publications, and pilot-license history use bounded stable-cursor pages. Firebase cleanup and player collection fallbacks visit bounded pages instead of assuming one unbounded result set. | `apps/api/src/database.pagination.test.js` pages 205 telemetry rows and asserts query plans use `client_telemetry_events_venue_time_idx` and `clients_venue_last_seen_idx`; Firebase cleanup pagination tests cover more than one page. |
| PERF-008 | The Player root clock is aligned to one-minute updates instead of rerendering the application tree every second. | Player presentation and lifecycle characterization tests pass. |
| PERF-009; 5.4, 5.12 | Public release metadata is a same-origin build manifest and uses normal cache semantics. The critical render path no longer depends on the GitHub API or a cross-origin `no-store` request. | `download-site/main.js`; `download-site/public/release-manifest.json`; `npm run download:build` passed. |
| PERF-010; 5.1, 6.2 | Express applies measured gzip compression except to SSE. Server timing and bounded redacted request-path/response-byte metrics are available without query strings. Player snapshot/mutation DTOs omit publication state, mutation IDs, duplicate flags, changed-entity counts, and other backend-only metadata. | Legal-page gzip and `Server-Timing` route assertions; `apps/api/src/routes/player.test.js`; full API route suite. |
| PERF-004, PERF-006; 5.2, 5.3, 5.19 | Changed state entities use chunked bulk upsert/delete operations. Firebase projection writes are batched within provider limits while the revision commit marker remains last; cleanup is paged. | Publisher test for 501 writes observes three bounded calls of 250, 250, and 1 before ordered marker completion; state architecture and pagination tests pass. |
| 5.5, 5.6 | Redacted request duration, response-size, retry, and failure instrumentation is bounded in memory and preserves request correlation. Exact isolated bundle, database-query, batch-size, API timing, compression, and end-to-end results are recorded here. | API performance route assertions, bounded Player request metrics, database query-plan tests, bundle-budget output, and isolated management smoke. Production timing collection was prohibited and is not claimed. |
| 5.17 | Desktop access/setup and Player identity/profile forms use persistent labels, native constraints, touched/blur validation, programmatic descriptions/alerts, and retained server validation. | Root and Player typechecks plus updated management/Player presentation characterization tests. |

Stage 4 dependency disposition:

- Added exact `compression@1.8.1` to the API for safe response compression, excluding event streams. The package is server-only; the API production dependency audit reported **0 vulnerabilities** after installation.
...
- Renderer bundle budget: **passed** at 558,814 bytes/172,314 gzip for the single initial JavaScript entry, with no source maps.
- Static download-site production build: **passed**.
- Isolated management browser smoke: **passed** for profile creation, table start, and seating.
- `git diff --check`: **passed**.
...
**Repository implementation state:** COMPLETED for the authorized Stage 6 source scope. VoiceOver and TalkBack device verification remains required before production promotion. Hallmarks remains **Founder Definition Required** and was not interpreted or implemented. Legal/company attribution and production-domain ownership/cutover remain deferred founder decisions.

| Finding / requirement | Implementation evidence | Verification |
| --- | --- | --- |
| DESIGN-001, DESIGN-002; 4.1, 4.3, 4.6 | `src/styles/00-foundation.css` and `player-app/src/styles/playerTheme.ts` now define the Astryx typography, supported weights, spacing, radii, elevation, borders, surfaces, colors, motion, density, states, icon sizes, breakpoints, and layout tokens. System fonts eliminate an unlicensed/unverified font-loading dependency, and all shipped weights are limited to 400-700. | `src/lib/astryxDesignSystem.test.ts`; root and Player strict TypeScript; desktop and Player presentation characterization tests. |
| DESIGN-005; 4.9-4.11 | Glass panels, backdrop blur, aurora/fog canvases, decorative blobs, ornamental grids, default glow, and broad decorative gradients were removed. Panels and controls now use solid tonal hierarchy and restrained borders. The poker-table surface retains bounded radial shading because it communicates felt/rail depth rather than decorating the application canvas. | Astryx prohibited-pattern source checks and rendered floor review at 1440 and 1180 pixels. |
| DESIGN-006; 4.12, 4.13 | Ordinary controls and cards use restrained Astryx radii and no default decorative shadow. Fully rounded geometry is reserved for status, tags, counts, timers, and circular controls; semantic state rings remain available where state must be immediately visible. | Token and prohibited-pattern tests; desktop build and smoke render. |
| DESIGN-007; 4.14, 4.17 | Repeated card/panel entrance animations, breathing text, flicker, pulse, and ornamental drag animation were removed. One bounded notification transition remains, with duration/easing tokens and reduced-motion suppression. Player interaction lift/scale is also suppressed when the operating system requests reduced motion. | Motion contract assertions; Player reduced-motion source coverage; desktop smoke and Player characterization tests. |
| DESIGN-008; 4.24, 4.29 | The management floor uses a scan-oriented primary work area and operational rail. Time Overview is a stable three-column table with bounded player/game text and full values exposed through titles; the rail stacks at the tested narrow-desktop threshold without horizontal overflow. | `tests/e2e/management-core-smoke.mjs` passed at 1440, 1180, 900, and 680 pixels with explicit column and overflow assertions; reviewed 1440/1180 renders. |
| DESIGN-009; 4.34, 4.38 | Player filters, plan selectors, and expandable game, membership, event, and saved-game surfaces expose their selected or expanded states programmatically. Icon-only desktop actions have accessible names and titles. | Astryx accessibility contract tests; Player strict TypeScript; management and Player presentation characterization tests. |
| DESIGN-010, A11Y-001, A11Y-002; 4.35-4.37, 4.39-4.43 | Persistent form labels and live-status behavior from Stages 3-4 are preserved. Modal, button, selected, expanded, and disabled semantics were added across Player interaction surfaces; desktop focus/state behavior remains keyboard-visible. Device screen-reader verification is explicitly separated from source-level completion. | Full Vitest discovery and strict typechecks; `docs/architecture/ASTRYX_DESIGN_SYSTEM.md` records the manual assistive-technology gate. |
| DESIGN-015 | ui.watermelon, Motion Primitives, and Haikei were evaluated against Orbit's existing React/Radix, React Native, performance, accessibility, and visual constraints. Watermelon's disclosure/live-feedback semantics were adopted through existing primitives; its registry code was not copied. Motion Primitives was not installed because existing platform motion and reduced-motion facilities cover the permitted motion. Haikei was rejected for P2 because its generated decorative vocabulary conflicts with the no-blob/no-aurora/no-noise direction. | Decision record and official-source links in `docs/architecture/ASTRYX_DESIGN_SYSTEM.md`; no dependency added. |
| DESIGN-016 | `scripts/brand-assets.cjs` defines the canonical Orbit logo/icon sources and byte-verifies six governed public/download/Player exports. ICO/favicon derivatives, logo spacing, colors, accessibility treatment, and replacement workflow are documented. | `npm run check:brand` passed; exact byte-comparison coverage in `src/lib/astryxDesignSystem.test.ts`. |
| 4.34-4.43 | The Astryx architecture record is the cross-surface contract for management desktop, Player native/web, API-hosted dashboard/legal surfaces, and the static public site. It includes skeleton, error, empty/loading/disabled/focus/state, density, responsive, icon, and logo rules rather than leaving those decisions implicit in page CSS. | Architecture-record review, ordered stylesheet digest coverage, and full production builds. |

Stage verification on 2026-08-11:

- Root aggregate TypeScript: **passed** across renderer, tests, Electron, and API.
- Player strict TypeScript: **passed**.
- Full Vitest discovery: **534/534 passed across 100 files**. An initial run had only the new filesystem-heavy brand-equality test exceed its default 5-second timeout while the other 533 tests passed; the byte-equality assertion was retained, its test-local timeout was raised to 20 seconds for OneDrive contention, and the complete gate passed on rerun.
- Desktop renderer production build: **passed**, 1,961 modules. Initial JavaScript remains within budget at 559.13 kB (172.36 kB gzip); CSS is 187.80 kB (33.60 kB gzip). The existing deferred ExcelJS evaluation and large-chunk warnings remain Stage 8 dependency/release evidence rather than hidden failures.
- Isolated management browser smoke: **passed** for profile creation, table start, seating, and Astryx responsive floor behavior at 1440, 1180, 900, and 680 pixels without horizontal overflow.
- Static public-site verification: **passed** for the existing seven-route Stage 5 surface.
- Brand governance check: **passed** for all six governed exports.
- `git diff --check`: **passed**.
- No dependency was installed for Stage 6. No production service, hosted endpoint, datastore, deployment, domain, DNS, registrar, certificate, canonical production hostname, legal attribution, or secret was accessed or changed. The tracked `data/orbit-api.sqlite3` artifact was not opened or modified. SEC-017 remains the human-only credential inventory, rotation, and removal action recorded in Stage 3.

### Stage 7 - Public Website Redesign

**Repository implementation state:** COMPLETED for every authorized, non-deferred Stage 7 requirement. Hallmarks remains **Founder Definition Required** and was not interpreted or implemented. Legal/company attribution and production-domain ownership/cutover remain deferred founder decisions and did not block repository-side public work.

| Finding / requirement | Implementation evidence | Verification |
| --- | --- | --- |
| DESIGN-003; 3.16, 3.22, 3.23; 4.4, 4.20 | `download-site/public/art/orbit-table-rhythm-v1.jpg` is production-quality generated abstract atmosphere with explicit non-product alt text. It contains no UI, people, venue, usage, metric, logo, or functional claim. `download-site/public/proof/orbit-core-empty-workspace.jpg` is a genuine current capture from the functioning management renderer. `scripts/capture-public-product-proof.mjs` reproduces it with Firebase disabled, an unreachable local API, zero player/customer/table records, and an explicit workspace redaction marker; it fails if smoke-fixture names appear. | Public asset/alt/provenance assertions; reproducible `npm run capture:public-proof`; rendered product review. The generated artwork prompt and the distinction between illustration and product evidence are recorded in `docs/architecture/PUBLIC_SITE.md`. |
| DESIGN-012; 4.19, 4.23 | `download-site/faq.html` adds a distinct seven-answer Orbit/Astryx FAQ using native `details`/`summary`, visible focus, programmatic open state, responsive layout, reduced-motion handling, and repository-backed answers about scope, requests, connectivity, identity, release state, platform, and deletion help. It is not copied from a stock component. | Public verifier asserts exact disclosure structure and unique metadata; Playwright opens the first disclosure by keyboard and renders the route at desktop/mobile widths. |
| DESIGN-013; 4.8, 4.9, 4.13, 4.17, 4.26 | The generic numbered feature grid was replaced with six uniform, restrained playing-card-inspired capability panels with code-native professional SVG symbols, clear reading order, selective state elevation, and a one-column mobile form. The composition is not an asymmetric bento layout and the metaphor is confined to marketing. | Desktop/mobile product renders; clean overflow checks; prohibited visual-pattern assertions. |
| DESIGN-014; 3.4 | Release copy continues to state that the installer is unavailable until an approved, verified promotion. The release-manifest loading state uses a bounded solid skeleton and retains meaningful static fallback text. No signing or release claim was added before Stage 8 hardening. | Same-origin manifest smoke assertion and public-source review. Existing legal entity/controller attribution was preserved. |
| SEO-001, SEO-002, SEO-006; 3.2, 3.5-3.14, 3.24 | FAQ is an independently indexable static route with unique title, description, canonical, Open Graph, Twitter, schema, sitemap, robots, and LLM discovery treatment. All eight public/error documents render meaningful HTML without API/Firebase availability; the configured-origin boundary remains unchanged and has no final production value. | `npm run check:public-site` passed for eight documents and six indexed routes; public browser smoke passed every route at 1440 and 390 pixels with clean console/network/page-error results. |
| 4.2, 4.5-4.7, 4.12, 4.15, 4.18, 4.21, 4.22, 4.24, 4.25, 4.27, 4.28, 4.31-4.33 | Public copy is specific to actual room operations. The site contains no testimonial/review marketing, fake UI, popularity metric, pricing cards, Trusted By band, logo carousel, emoji icons, cursive copy, animated grid, glow, gradient blob, aurora, noise overlay, or “built for the future” claim. Motion is limited to one hero-art settle and FAQ disclosure feedback, both suppressed by reduced-motion preferences. | Source prohibitions in `scripts/verify-public-site.mjs`; desktop/mobile render review; emitted CSS scan. |
| 3.1 / DESIGN-011 | No Hallmarks page, route, schema, copy, or meaning was added. | **Founder Definition Required.** |

Stage verification on 2026-08-11:

- Reproducible current product proof capture: **passed** from an isolated zero-record local workspace; no customer/player/private data or production service was accessed.
- Public static verification: **passed** for eight HTML documents, six indexed routes, metadata/schema/discovery, generated illustration and real capture evidence, FAQ structure, marketing/visual prohibitions, configured preview origin, and no source maps. The first new verifier run correctly exited nonzero because an over-broad negative-phrase assertion matched the page's explicit “not fictional” disclaimer; the assertion was narrowed to prohibited claims, not disclaimer language, and the complete verifier passed on rerun.
- Isolated public browser smoke: **passed** for all eight routes at 1440x900 and 390x844, with keyboard-operable FAQ disclosure, loaded current product capture, correct release fallback, no horizontal overflow, and clean page/console/network results. Home, Product, FAQ, and mobile Home renders were reviewed.
- Root aggregate TypeScript: **passed** across renderer, tests, Electron, and API.
- Player strict TypeScript: **passed**.
- Full Vitest discovery: **534/534 passed across 100 files**.
- Desktop renderer production build: **passed**, 1,961 modules. The public build emits a 1.95 kB JavaScript entry (0.95 kB gzip) and 12.92 kB CSS (3.50 kB gzip), plus the optimized 165,624-byte generated illustration and 68,575-byte current product capture.
- No dependency was added. No production site, hosted endpoint, datastore, Firebase project, release, deployment, legal attribution, domain, DNS, registrar, certificate, canonical production hostname, production client link, or secret was accessed or changed. The tracked `data/orbit-api.sqlite3` artifact was not opened or modified. SEC-017 remains the human-only credential action recorded in Stage 3.

### Stage 8 - Production Hardening and Final Verification

**Repository implementation state:** COMPLETED for the authorized Stage 8 repository scope. No release, artifact publication, protected-environment approval, deployment, signing credential, production service, or domain was exercised. The owned external alert destination and a signed canary/rollback rehearsal still require explicitly authorized external environments; their absence is not represented as proof that the future operational safeguards have run. Refactor P2 as a whole remains incomplete because SEC-017 still requires human credential inventory/rotation/removal and Hallmarks remains **BLOCKED BY FOUNDER DEFINITION**. Legal/company attribution and production-domain ownership/cutover remain **DEFERRED BY FOUNDER**.

| Finding / requirement | Implementation evidence | Verification |
| --- | --- | --- |
| REL-005, DESIGN-014; 6.13 | `.github/workflows/release.yml` is manual-only and accepts an exact source SHA, new semantic version, canary/stable channel, standard/rollback reason, and separate promotion choice. Candidate signing and production promotion use distinct protected environments. The candidate runs all gates, requires Authenticode, emits checksums/source metadata, creates a GitHub provenance attestation, and uploads a uniquely named non-overwritable artifact. `forceCodeSigning` and executable signing fail closed; the only release build script uses `--publish never`. Canary promotion is a prerelease and normal desktop clients reject prerelease discovery. Rollback is a new higher version from an exact known-good commit with migration/revision rehearsal, not an unsafe binary or schema downgrade. | `npm run check:release-controls`; `docs/operations/RELEASE_AND_ROLLBACK.md`. The workflow was deliberately not dispatched, so no signed artifact or release claim was fabricated. |
| REL-005; explicit update timing | A downloaded update no longer invokes installation or installs on ordinary app exit. The trusted main process exposes current status and a bounded explicit-install IPC command; the preload bridge and Astryx shell show **Install update and restart** only when appropriate. Installation requests a renderer-state flush and blocks restart after a missing, failed, or timed-out acknowledgement. | Electron update/controller, main/preload composition, and AppShell tests cover no automatic install, explicit approval, successful preservation/restart, and fail-closed preservation. |
| REL-008, REL-012 | CI now installs all lockfiles and runs sensitive-path, dependency, release-control, module-boundary, complete verification, renderer budget, public/brand, and both critical Playwright gates. Both browser harnesses build and preview production bundles. Management fixture access is possible only in a build explicitly bound to disabled Firebase, the isolated loopback API, and a loopback browser origin; hosted and packaged origins are rejected. | Production management and public smoke passed with zero page, console, request, or responsive-overflow failures. The isolated fixture-boundary negative tests pass. |
| REL-012; platform-independent verification | The recurring Stripe CommonJS compiler assertion retained all behavior but receives a test-local 20-second filesystem-contention allowance. The first final full run reproduced the known OneDrive timeout at 8.51 seconds; the complete rerun passed. A first module audit also exposed four real database-to-HTTP imports. Shared redaction and alert ownership moved to `apps/api/src/operations/`, with compatibility shims at the prior HTTP paths. | `npm run audit:module-graph` passes across 188 source files and 550 relative edges with zero cycles, dependency violations, or unresolved imports. `npm run verify` passes 538/538 tests across 101 files. |
| REL-010; 6.12, 6.24 | Stage 4's bounded redacted server/auth/publication alert behavior and host-allowlisted HTTPS delivery remain intact under the lower-level operations boundary. Release verification/environment-approval failure is now an explicit incident signal, and the release runbook defines owner escalation and immutable recovery. | Operational-alert, state-publication, and database behavior tests pass; API strict check passes. `ORBIT_ALERT_WEBHOOK_URL` ownership/delivery remains externally unverified and is not marked as an exercised production safeguard. |
| 6.21 | Stage 3/4 trusted-proxy-aware endpoint quotas, authentication-abuse signals, text-batch limiting, SSE connection capacity, and bounded retry behavior remain enforced. No release or CI change bypasses them. | Full API/security test discovery passes as part of 538/538; production smokes use only isolated loopback services. |
| DEP-001 | No blanket or forced audit fix was used. Compatible overrides move `brace-expansion` to 5.0.9, `js-yaml` consumers to compatible 3.15.1/4.3.1 lines, `nanoid` to 3.3.17, and Player `fast-uri` to 3.1.5. Root/API/Player production audit counts move from 4/0/16 High to 0/0/11 High. The remaining Player entries are one Expo 54 / React Native 0.81 Metro `image-size` build-tool chain with no compatible upstream repair; incompatible Expo/React Native/Purchases downgrades were rejected. A dated policy fails for new names/severity, every Critical, or after 2026-09-30. | `npm run security:dependencies`; `config/dependency-audit-policy.json`; `docs/architecture/DEPENDENCY_SECURITY.md`. Root and API production graphs report zero; Player reports the exact eleven reviewed entries. This review narrows current reachability and does not claim the future upstream safeguard is complete. |
| 3.17-3.19 | Desktop and public Vite production builds explicitly disable source maps. The existing renderer budget gate remains below its 650 kB / 205 kB-gzip initial-entry limits; public static verification rejects emitted maps. Production-mode browser checks now fail on page errors, console errors, and failed requests. | Renderer initial JavaScript: 560,566 bytes / 172,814 gzip. Public entry: 1.95 kB / 0.95 kB gzip. `npm run check:renderer-bundle`, `npm run check:public-site`, and both production smokes pass. |

Stage verification on 2026-08-11:

- Lockfile installs: root, API, and Player `npm ci` each passed. The root all-dependency install still reports three development/package-tool advisories; the production-only policy reports zero root findings and does not conflate development absence with a future safeguard.
- Canonical verification: **passed**. Root renderer/test/Electron/API strict checks passed; Player strict TypeScript passed; **538/538 tests across 101 files** passed; the 1,961-module renderer production build passed.
- Renderer bundle budget: **passed** at 560,566 bytes / 172,814 gzip for the initial JavaScript entry, with no emitted source maps. ExcelJS remains deferred at 1,066,528 bytes and the existing dependency `eval` warning remains visible rather than suppressed.
- Public static verification and production browser smoke: **passed** for eight pages, six indexed routes, metadata/schema/discovery/source-map controls, desktop/mobile rendering, keyboard FAQ behavior, loaded real proof, and a clean page/console/network result.
- Management production-bundle browser smoke: **passed** for profile creation, table start, four seating transitions, responsive floor behavior, and a clean page/console/network result against only an isolated loopback API with Firebase disabled.
